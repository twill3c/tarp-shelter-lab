...
| L8 | 図鑑の拡張(鋤先・ティピーを追加して五種) | 完了 |
| L9 | Lean-To の開口の向きの修正と、開口を幾何で検算するゲート | 完了 |
| L10 | 居住性(第三の軸)。広さを問う課題とクイズ | 完了 |
...
モデルが測るのは三つの軸である。**耐風**(正面投影面積と開口の向き)、
**排水**(屋根の勾配と張り)、**居住性**(座って頭が当たらない高さが取れる床の広さ)。
耐風と排水だけでは前が大きく開く型の長所が測れず、どの設問でも勝てない型が出た。
第三の軸を足して、それを解消した。
...
- シェルター定義は `data/shelters.json`(五種)。張り終えた形の寸法は三平方で導いている
- **型を足すと課題とクイズの表が壊れる。それが正しい** —— 落ちたら実測し直して SPEC の表を書き換える
